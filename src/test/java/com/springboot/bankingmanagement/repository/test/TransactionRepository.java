 ...
 * 
 * @Query(value =
 * "SELECT * FROM statement_history s WHERE s.account_number = :accountNumber AND MONTH(date) = :month AND YEAR(date) = :year"
 * , nativeQuery=true) List<Statement>
 * getStatementHistory(@Param("accountNumber") Long
 * accountNumber,@Param("month") String month,@Param("year") String year);
 * 
 * }
 */